# Framework

Sado Investment Labの投資判断を支える、長期的な思想・原則・評価方法をまとめるディレクトリです。

> **Sado Investment Labは、利益を追うだけの投資手法ではない。**
>
> **市場との対話を通じて、市場を理解し、企業を理解し、自分自身の投資理論を磨き続けるための研究所である。**

## Mission

企業の実力と市場参加者の期待をそれぞれ分析し、両者のギャップから投資利益を得ることを目指す。

Frameworkの思想、評価、指標、検証は、すべてこの投資判断を改善するために存在する。

## Files

- [01_Investment_Philosophy.md](01_Investment_Philosophy.md): 長期的な投資思想、投資を行う目的、手法や時間軸との向き合い方
- [02_Market_Psychology.md](02_Market_Psychology.md): 市場参加者の期待、恐怖、需給の理解
- [03_Thinking_Process.md](03_Thinking_Process.md): 分析から判断までの思考手順
- [04_Investment_Rules.md](04_Investment_Rules.md): 売買とリスク管理のルール
- [05_Evaluation_Framework.md](05_Evaluation_Framework.md): 企業を評価する共通の物差し
- [06_Capital_Allocation.md](06_Capital_Allocation.md): 期待値に基づく資金配分の考え方
- [07_Lessons_Learned.md](07_Lessons_Learned.md): 投資から得た再利用可能な教訓
- [08_Original_Metrics.md](08_Original_Metrics.md): PERを補完する独自指標と検証履歴

## Templates

- [Theme_Template.md](../07_Templates/Theme_Template.md): テーマ分析を統一形式で記録するテンプレート

## Usage

1. 投資判断の前に思想とルールを確認する
2. 企業価値と市場心理を分けて分析する
3. Thinking Processに沿って事実・解釈・仮説を整理する
4. Evaluation Frameworkで同じ基準から評価する
5. 必要に応じてOriginal Metricsで成長性と市場期待を補助評価する
6. Capital Allocationで資金の配分を決める
7. 結果をLessons Learnedへ戻し、Frameworkを育てる

---

## Philosophy

このFrameworkは完成形ではない。

市場は変わる。社会は変わる。技術も変わる。

したがって、このFrameworkも経験と検証を通じて進化し続ける。

私は市場を完全に予測することを目指さない。

市場を継続して観察し、対話し、その時々の変化を理解することを目指す。

企業を分析するだけでなく、実際の市場参加を通じて感覚を磨き、得られた経験を投資理論へ戻していく。

この循環を続けることが、Sado Investment Labの目的である。
